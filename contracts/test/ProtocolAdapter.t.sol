...
contract ProtocolAdapterTest is Test {
    ...
    function setUp() public {
        ...
        _pa = new ProtocolAdapter(_router, _MOCK_VERIFIER_SELECTOR);

        _fwd =
            address(new ForwarderExample({protocolAdapter: address(_pa), calldataCarrierLogicRef: _CARRIER_LOGIC_REF}));
    ...
    function test_constructor_reverts_on_vulnerable_risc_zero_verifier() public {
        ...
        new ProtocolAdapter(_router, _MOCK_VERIFIER_SELECTOR);
    }

    function test_execute_reverts_on_vulnerable_risc_zero_verifier() public {
        // Deploy the contracts with a proper verifier
        (_router, _emergencyStop,) = new DeployRiscZeroContracts().run();

        // Grab the proper selector from the Risc0 Utils
        _pa = new ProtocolAdapter(_router, RiscZeroUtils._SELECTOR);
    ...
    function test_execute_reverts_on_pre_existing_nullifier() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(NullifierSet.PreExistingNullifier.selector, preExistingNf), address(_pa));
        _pa.execute(tx2);
    }
