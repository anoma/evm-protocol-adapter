...
abstract contract ForwarderBase is IForwarder {
    ...
    function forwardCall(bytes32 actionTreeRoot, bytes calldata input) external returns (bytes memory output) {
        _checkCaller(address(_PROTOCOL_ADAPTER));

        output = _forwardCall({actionTreeRoot: actionTreeRoot, input: input});
    }
