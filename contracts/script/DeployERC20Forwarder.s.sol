...
import {PROTOCOL_ADAPTER_VERSION} from "../src/ProtocolAdapter.sol";
...
contract DeployERC20Forwarder is Script {
    function run(address protocolAdapter, address emergencyCommittee, bytes32 carrierLogicRef) public {
        vm.startBroadcast();
        new ERC20Forwarder{salt: sha256(bytes(string.concat("ERC20Forwarder", " ", PROTOCOL_ADAPTER_VERSION)))}({
